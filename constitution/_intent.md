...
immigrate to a thriving one. All of this is driven by each citizen's
...
4. **Pure engine, impure shell** — `packages/simulation` contains no React/IndexedDB/DOM code; `packages/web` owns all I/O (storage, UI) and calls into `packages/simulation` for calculations.
